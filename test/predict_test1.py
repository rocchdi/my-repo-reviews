...
r = requests.get(
        url='http://administrateur:password@{address}:{port}/'.format(address=api_address, port=api_port) 
)
...
if (test_status == 'SUCCESS'): #api fonctionne
    ...
    r = requests.get(
        url='http://administrateur:password@{address}:{port}/predict'.format(address=api_address, port=api_port),
        params= {
